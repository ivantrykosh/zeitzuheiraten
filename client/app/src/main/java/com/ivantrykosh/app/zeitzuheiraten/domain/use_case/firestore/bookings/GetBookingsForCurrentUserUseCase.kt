...
class GetBookingsForCurrentUserUseCase @Inject constructor(
    ...
    operator fun invoke(startAfterLast: Boolean, pageSize: Int) = flow<Resource<List<Booking>>> {
        try {
            emit(Resource.Loading())
            val userId = userAuthRepository.getCurrentUserId()
            val bookings = bookingRepository.getBookingsForUser(userId, startAfterLast, pageSize)
