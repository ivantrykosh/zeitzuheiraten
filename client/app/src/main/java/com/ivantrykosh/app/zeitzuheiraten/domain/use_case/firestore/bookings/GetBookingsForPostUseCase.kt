...
class GetBookingsForPostUseCase @Inject constructor(
    ...
    operator fun invoke(postId: String, startAfterLast: Boolean, pageSize: Int) = flow<Resource<List<Booking>>> {
        try {
            emit(Resource.Loading())
            val bookings = bookingRepository.getBookingsForPost(postId, startAfterLast, pageSize)
